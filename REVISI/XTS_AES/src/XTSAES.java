...
public class XTSAES {
    ...
    public Result XTSAESEnc(byte[] P, byte[] keys, int blocksize) {
        ...
        int b = P.length % 16;
        int length=(C.length-1)*16;
        System.out.println("lebih enc"+b);
        if (b == 0) {
    ...
    public Result XTSAESDec(byte[] C, byte[] keys, int blocksize) {
        ...
        int length=(P.length-1)*16;
        int b = C.length % 16;
        System.out.println("lebih dec"+b+C.length);
        if (b == 0) {
            ...
            P[blocksize - 1] = XTSUtils.takeFirstBits(PP, b);
